...
//        }
        .onReceive(NotificationCenter.default.publisher(for: .programParameterUpdated)) { notification in
            debugPrint(icon: "❎", message: "received notification: data: \(notification.userInfo?.debugDescription)", type: .trace)
...
//                  let source = ModulationSource(rawValue: value),
                ...
                debugPrint(icon: "❌❎", message: "Not a Modulation Source Selector. Skipping.\n \(notification.userInfo?.debugDescription)", type: .trace)
                return
            }

            debugPrint(icon: "💕", message: "type: \(type), value: \(value), valueType: \(valueType)", type: .trace)
            
            Task { @MainActor in
                if case .off = valueType {
                    debugPrint(icon: "📤", message: "\n -----> Turning off Modulation Source: \(type), self is: \(self.type)", type: .trace)
                    self.destinations.removeAll { $0 == type }
                }
                else if valueType.relatedSources.contains(valueType){
                    debugPrint(icon: "✅", message: "Adding Modulation: \(type), self is: \(self.type)", type: .trace)
                    destinations.append(type)
                    debugPrint(icon: "✅", message: "Modulation Destinations: [\(destinations)]", type: .trace)
                }
            }
