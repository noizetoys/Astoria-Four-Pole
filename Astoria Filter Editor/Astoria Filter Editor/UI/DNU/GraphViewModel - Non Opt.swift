...
//                print("CC:  channel: \(ccData.channel), config Channel: \(config.midiChannel), cc: \(ccData.cc), breath:  \(ContinuousController.breathControl)")
                
                if ccData.cc == ContinuousController.breathControl {
                    self.ccValue = ccData.value
                        //                    ccData.channel == config.midiChannel
                }
                else {
                    print("\n --> Data not good: cc = \(ccData.cc), should be \(ContinuousController.breathControl), channel: \(ccData.channel) should be \(config.midiChannel)")
                }
...
//                print("Note:  channel: \(note.channel), config Channel: \(config.midiChannel), note: \(note.note), velocity: \(note.velocity)")
...
//                }
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { [weak self] _ in
            Task { @MainActor in
                ...
                if currentNoteValue != self.lastNoteValue {
