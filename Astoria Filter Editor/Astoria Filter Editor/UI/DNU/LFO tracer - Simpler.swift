...
struct LFOTracerView: View {
    ...
    @State var musicalNote: MusicalNote = .init(noteName: "C", octave: 3, cents: 0, midiNote: 48)
    ...
    @State private var isRunning: Bool = true
        ...
        /// When true, uses simplified rendering for better performance
    @State private var performanceMode: Bool = false
    
        /// Frame rate for the animation (default 30 FPS for better performance)
        /// Can be increased to 60 FPS if needed, but 30 is sufficient for LFO visualization
    private let animationFrameRate: Double = 30.0
    
        /// Number of points used to draw the waveform (reduced for better performance)
        /// 200 points is sufficient for smooth curves while being much faster than 500
    private let waveformPoints: Int = 200
    ...
    private var performanceModeToggle: some View {
        HStack(spacing: 8) {
            Image(systemName: performanceMode ? "hare.fill" : "tortoise.fill")
                .foregroundColor(performanceMode ? .orange : .gray)
                .font(.caption)
        ...
        .help("Performance Mode: Disables visual effects for better performance")
    }
    ...
    private func waveformContent(geometry: GeometryProxy) -> some View {
        ZStack {
            ...
            staticWaveformPath(geometry: geometry)
                ...
                // Only render glow and trail in normal mode for better performance
            if !performanceMode {
    ...
    private func staticWaveformPath(geometry: GeometryProxy) -> some View {
        Path { path in
            ...
            let amplitude = height * 0.4
                ...
                // Use optimized point count for better performance
            for i in 0..<waveformPoints {
                let x = (Double(i) / Double(waveformPoints)) * width
                let localPhase = (Double(i) / Double(waveformPoints)) * 2 * .pi
    ...
    private func tracerTrail(geometry: GeometryProxy) -> some View {
            // PERFORMANCE: Reduced from 5 to 3 trail dots for better performance
            // Each trail dot requires position calculation and rendering
        ForEach(0..<3, id: \.self) { index in
            ...
            if trailPhase >= 0 {
                ...
                Circle()
                    ...
                    .frame(width: 10 - Double(index) * 2, height: 10 - Double(index) * 2)
    ...
    private func frequencySliderBinding() -> Binding<Double> {
        Binding(
            ...
            set: { newValue in
                ...
                let replacement = snapToNote ? snapFrequencyToNote(newFrequency) : newFrequency
                setFrequency(to: replacement)
            }
        )
    }
...
//            periodAndValueDisplay
    ...
    func startAnimation() {
        ...
        timer?.invalidate()
            ...
            // CREATE AN OPTIMIZED ANIMATION TIMER
            // ═══════════════════════════════════════════════════════════════════════
            // Using 30 FPS instead of 60 FPS reduces CPU usage by ~50% with minimal
            // visual difference for LFO visualization (LFOs are typically slow)
            //
            // Frame time = 1 / animationFrameRate
            // At 30 FPS: ~33.33 milliseconds per frame
            // At 60 FPS: ~16.67 milliseconds per frame
        let frameTime = 1.0 / animationFrameRate
        
        timer = Timer.scheduledTimer(withTimeInterval: frameTime, repeats: true) { [weak timer] _ in
            ...
            guard isRunning else { return }
                ...
                // ═══════════════════════════════════════════════════════════════════
                // Adjusted for the current frame rate
                // deltaPhase = 2π × frequency × frameTime
            let deltaPhase = 2 * .pi * frequency * frameTime
            ...
            phase += deltaPhase
                ...
                // ═══════════════════════════════════════════════════════════════════
                // Use modulo to wrap phase back to 0-2π range
                // This is much faster than checking and subtracting
            if phase >= 2 * .pi {
            ...
            if selectedWaveform == .sampleHold {
                ...
                let samplePhaseInterval = 2 * .pi * sampleRate * frameTime
